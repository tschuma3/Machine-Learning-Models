...
(train_data, train_labels), (test_data, test_labels) = data.load_data(num_words=10000) #num_words=10000 is only getting the 10000 most frequent words
...
train_data = keras.preprocessing.sequence.pad_sequences(train_data, values=word_index["<PAD>"], padding="post", maxlen=250)
test_data = keras.preprocessing.sequence.pad_sequences(test_data, values=word_index["<PAD>"], padding="post", maxlen=250)
...
model.add(keras.layers.Embedding(10000, 16)) #Group words that are similar ---- math version explination: finds word vectors for each word that is passed in and passes those vectors to the next layer
